<script lang="ts">
    ...
    let classCode = "S12FT4";

    function handleContinue() {
        // Logic for creating class would go here
        isSuccess = true;
    }
...
<Dialog.Root bind:open onOpenChange={(val) => { if (!val) handleClose(); }}>
	<Dialog.Content class="sm:max-w-[700px] p-8 gap-6">
        {#if !isSuccess}
            <Dialog.Header>
                <Dialog.Title class="text-[24px] font-bold text-[#252424]">Buat Kelas Baru</Dialog.Title>
            </Dialog.Header>
            <div class="grid gap-6 py-4">
                <div class="grid gap-3">
                    <Label for="name" class="text-[16px] font-semibold text-[#595959]">Nama Kelas</Label>
                    <Input
                        id="name"
                ...
                <div class="grid gap-3">
                    <Label for="description" class="text-[16px] font-semibold text-[#595959]">Deskripsi Mata Pelajaran</Label>
                    <Textarea
                        id="description"
        ...
        {:else}
            <div class="flex flex-col items-center justify-center py-6 gap-6">
                ...
                <div class="text-center space-y-2">
                    <h2 class="text-[28px] font-bold text-[#252424]">Kelas Berhasil Dibuat!</h2>
                    <p class="text-[16px] text-[#252424]">Bagikan kode kelas ini kepada siswa Anda</p>
                </div>

                <div class="bg-[#f0f0f0] px-4 py-2 rounded-[8px] flex items-center gap-3">
                    <Copy class="size-4 text-[#8c8c8c]" />
                    <span class="text-[18px] font-bold text-[#8c8c8c] tracking-wider">{classCode}</span>
                </div>
